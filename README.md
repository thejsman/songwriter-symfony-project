...
- Server: [Nginx](https://nginx.org/en/) or [Apache](https://httpd.apache.org/docs/2.4/)
- php: >= 7.1
- framework: [Symfony 3.3](https://symfony.com/doc/3.3/setup.html)

Docs
--------
/api/doc

REST API documentation with detailed description for each endpoint.
To generate documentation you have to install:

2. Install [nvm (node version manager)](https://github.com/nvm-sh/nvm#installation-and-update)
3. Build API sandbox 
    1. Go to docs folder `cd docs`
    2. Install packages form package.json `npm install`
    3. Rename environment config env.dist `cp .env.dist .env`
    4. Define environment parameters: 
       - example for base url for sandbox`API_BASE_URI=http://songwriter-api.loc/`
       - example for web sockets port `WS_PORT=3001`
       - example for web sockets auth uri `WS_URI=http://songwriter-api.loc`
    5. Build sandbox documentation `gulp build`


- [Installation for development](#installation-for-development)
- CI/CD for production environment
    - [Installation](#installation)
    - [Updating](#updating)
    
## Installation for development
1. `git clone git@{repository_domain}/api.git`
2. Run `composer install` ([download](https://getcomposer.org/download/))
3. Create database schema:
    1. for main DB `php bin/console doctrine:schema:create`
4. Load fixtures `php bin/console doctrine:fixtures:load --fixtures=src/AppBundle/DataFixtures/Demo`
5. In file of the server configuration root dir should leads to {PATH}/web
    1. DirectoryIndex app_dev.php
    2. If Apache is used `cp .htaccess.dist .htaccess`
...
2. Run `composer install` ([download](https://getcomposer.org/download/))
3. Create database schema:
    1. for main DB `php bin/console doctrine:schema:create --env=prod`
4. In file of the server configuration root dir should leads to {PATH}/web
    1. DirectoryIndex app_stage.php
    2. If Apache is used `cp web/.htaccess.dist web/.htaccess`

### Updating
1. `git pull origin master`
2. Run `composer install` ([download](https://getcomposer.org/download/))
3. Update database schema:
    1. for main DB `php bin/console doctrine:schema:update --force`
4. Remove previous cache `rm -rf var/cache/*`
...
3 Add `'access_attribute'` to service mconfig for set attributes to check user permissions (as needed). \
...
```
# src/AppBundle/Resources/mconfig/services.yml
